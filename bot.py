...
async def guide():
    ...
    embed.add_field(name="-report", value="Report quests from Pokestops as follows: /n -report <pokestop name>, <pokestop location>, <quest notes> /n  Ex) -report Doyle Park, 61st and hollis, Spinda", inline=False)
    await bot.say(embed=embed)
...
async def report(ctx, stopInfo:str):
...
#{ctx.message.author.mention}
   ...
   await ctx.bot.reply("thanks for reporting the quest!  Type '-sheet' (without quotes) to see today's quests.")
